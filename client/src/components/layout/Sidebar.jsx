...
const activeClass = "block px-4 py-2 rounded-lg bg-blue-100 text-blue-700 font-semibold";

function SidebarItem({ to, children }) {
  return (
    <NavLink to={to} className={({ isActive }) => (isActive ? activeClass : linkClass)}>
      {children}
    </NavLink>
  );
}
...
export default function Sidebar() {
  ...
  return (
    <aside className="w-64 bg-white shadow-md min-h-screen p-4">
      <h2 className="text-xl font-bold text-gray-800 mb-6">Opportunity Portal</h2>
      ...
      <nav className="space-y-2">
        ...
        {/* USER */}
        {role === "user" && <SidebarItem to="/applications">My Applications</SidebarItem>}

        {/* CONTRIBUTOR + ADMIN */}
        {(role === "contributor" || role === "admin") && (
          <>
            <SidebarItem to="/submissions">My Submissions</SidebarItem>
            <SidebarItem to="/submit">{role === "admin" ? "Add Opportunity" : "Submit Opportunity"}</SidebarItem>
            <SidebarItem to="/contributor-analytics">Analytics</SidebarItem>
          </>
        )}

        {/* ADMIN ONLY */}
        {role === "admin" && <SidebarItem to="/moderation">Moderation</SidebarItem>}
