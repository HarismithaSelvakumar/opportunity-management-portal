...
export default function AdminDashboard() {
  ...
  if (loading) return <div className="bg-white p-6 rounded-xl shadow">Loading admin dashboard...</div>;
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
        <div>
          ...
          <p className="text-gray-600 mt-1">Platform overview and application activity.</p>
        </div>

        <div className="flex gap-3">
          <Link to="/moderation" className="px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 text-sm font-medium">
            Go to Moderation
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        ...
        <Card title="Total Opportunities" value={totals.totalOpportunities || 0} />
        <Card title="Total Applications" value={totals.totalApplications || 0} />
      </div>

      {/* Status counts */}
      <div className="bg-white shadow rounded-2xl p-6">
        <h2 className="text-lg font-semibold text-gray-900">Application Status Counts</h2>
              ...
              <tbody>
                {recent.map((a) => (
                  <tr key={a._id} className="border-t">
                    <td className="p-3">
                      {a.userId?.name || "-"} <span className="text-gray-500">({a.userId?.email || "-"})</span>
                    </td>
                    <td className="p-3">
                      {a.opportunityId?.title || "-"}{" "}
                      <span className="text-gray-500">• {a.opportunityId?.company || "-"}</span>
                    </td>
